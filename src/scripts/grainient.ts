// Grainient – körniger, animierter Farbverlauf (WebGL2 via ogl).
// Vanilla-Port der ReactBits-Komponente (https://reactbits.dev/backgrounds/grainient,
// MIT + Commons Clause, © David Haz). Shader unverändert, React-Hülle durch
// mountGrainient() ersetzt.
...
/**
 * Hängt den Grainient-Canvas in `container` und startet die Render-Schleife.
 * Pausiert automatisch, wenn der Container nicht im Viewport ist oder der Tab
 * im Hintergrund liegt. Gibt eine Cleanup-Funktion zurück.
 */
export function mountGrainient(container: HTMLElement, opts: GrainientOptions = {}): () => void {
    const o = {
        timeSpeed: 0.25, colorBalance: 0, warpStrength: 1, warpFrequency: 5, warpSpeed: 2,
        warpAmplitude: 50, blendAngle: 0, blendSoftness: 0.05, rotationAmount: 500, noiseScale: 2,
        grainAmount: 0.1, grainScale: 2, grainAnimated: false, contrast: 1.5, gamma: 1, saturation: 1,
        centerX: 0, centerY: 0, zoom: 0.9, color1: '#FF9FFC', color2: '#5227FF', color3: '#B497CF',
        lightMode: false,
        ...opts,
    };
    ...
    const program = new Program(gl, {
        ...
        uniforms: {
            ...
            uGrainAnimated: { value: o.grainAnimated ? 1 : 0 },
